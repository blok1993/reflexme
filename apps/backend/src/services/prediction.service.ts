...
export async function generateWeeklyPatternsLLM(input: {
  ...
  const systemPrompt = [
    ...
    // ── Примеры ────────────────────────────────────────────────────────────────
    'ХОРОШО: "Ты чаще ждёшь конфликта там, где его потом нет." / "В дни тревоги ловушки почти всегда сбываются." / "Когда пишешь мало утром, прогнозы точнее."',
    'ПЛОХО: "Прослеживается тенденция к ожиданию конфликтных ситуаций, которые в действительности не реализуются." / "В тревожные дни наблюдается повышенный уровень реализации предупреждений."',
...
export async function generatePatternCardsLLM(input: {
  ...
  const systemPrompt = [
    ...
    '— Ожидал одно, получил другое (прогноз не сбылся в конкретных условиях)',
    '— Что-то всегда работает, но человек этого не ценит',
    '— Что-то никогда не работает, но человек всё равно на это надеется',
    '— Паттерн, который повторяется, но сам человек его не видит',
    '— Триггер который запускает плохой день — конкретный, не очевидный',

    // ── Формат ────────────────────────────────────────────────────────────────
    'Insight: 1-2 предложения. Максимум 30 слов. Начинай с наблюдения — не с вывода.',
    'Highlight: конкретная цифра или факт. Без слова "дней" в конце если уже есть число.',
    ...
    // ── Примеры хорошего ──────────────────────────────────────────────────────
    'ХОРОШО: "Ты идёшь на людей за энергией — но после почти каждого такого дня тебе хуже." / "Советы о силе не работают именно тогда, когда ты в них больше всего нуждаешься — в тревожные дни." / "Ты точнее предсказываешь события дня, чем ловушки в нём."',
  ...
  const humanReadableReviews = input.reviews.map((r) => ({
    ...
    предупреждение_о_ловушке: SCORE_RU[r.trapWarningScore] ?? r.trapWarningScore,
